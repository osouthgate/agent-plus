...
<p align="center">
  <a href="https://youragentplus.xyz/#lifecycle">
    <img src="./assets/lifecycle-simple.png" alt="agent-plus skill lifecycle — parallel discovery (repo-analyze + session mining) → propose → scaffold → use → improves each session" width="100%">
  </a>
  <br>
  <sub><i>→ <a href="https://youragentplus.xyz/#lifecycle">Interactive version with commands at youragentplus.xyz/#lifecycle</a></i></sub>
</p>
...
**What the script does:** detects your shell + OS, resolves the target release tag, and downloads the tarball. For a tagged release it prefers a self-built release-asset tarball and verifies its SHA-256 against a `SHA256SUMS` file published on that same release — a mismatch aborts the install. It then installs the five primitives as CLI tools under `~/.local/share/agent-plus/` (with wrapper shims in `~/.local/bin/`), then chains into `agent-plus-meta init` (idempotent, runs the wizard). **On a real install** (not `--dry-run`), you need `curl`, `tar`, and **`python3` on PATH** — the CLI wrappers invoke Python. **`--dry-run`** only prints the plan (including whether verification applies to the resolved tag) and does not require `curl`, `tar`, or `python3`. To register them as Claude Code plugins so Claude can invoke them directly, also run the **Manual install** commands below — or skip `install.sh` and use Manual install only.
...
**Tarball verification:** tagged installs download a self-built source tarball — not GitHub's auto-generated `archive/refs/tags/*.tar.gz`, which is not guaranteed byte-stable long-term — and check its SHA-256 against a `SHA256SUMS` file uploaded to the same release. A checksum mismatch is a hard failure: the install aborts (exit 1, even under `--unattended`) and the downloaded tarball is discarded. `SHA256SUMS` and the tarball are both GitHub release assets, so this is same-origin verification, not an independent second trust root — a separately-hosted checksum mirror is possible future hardening, not something this implements today. Releases before v0.21.0 predate this and fall back to the unverified auto-archive with a stderr/console warning; `main`/`master` installs are always unverified by design (moving target — pin a release via `AGENT_PLUS_VERSION` / `$env:AGENT_PLUS_VERSION` for a verified install). Set `AGENT_PLUS_NO_VERIFY=1` (`$env:AGENT_PLUS_NO_VERIFY` on Windows) to skip verification explicitly.
