...
### Changed
- **Storage precedence docs realigned to the real 2-tier resolution.** `bin/skill-feedback`'s module docstring, README's "Storage resolution" section, and SKILL.md's storage section all still described the stale 4-tier precedence (env -> git-toplevel -> cwd -> home) that stopped being true in v0.19.4. All three now document the real `env -> home` resolution plus its rationale ("ratings reflect skill quality, not a specific repo"). README's `path` output description is corrected from `source: env|git|cwd|home` to `source: env|home`, and the stale "gitignore it for personal feedback, commit it to share with the team" advice is removed -- dead now that storage isn't project-local. Both docs now point at `SKILL_FEEDBACK_DIR` as the explicit escape hatch for a repo-scoped or team-shared store.
- **`path` gains a `legacy_store` field.** Non-null (absolute path, string) when a pre-v0.19.4 project-local `.agent-plus/skill-feedback/` directory is still sitting nearby with at least one `.jsonl` file in it and differs from the active storage root; `null` otherwise. Repurposes the previously-dead `_git_toplevel()` helper (zero call sites since the v0.19.4 migration) instead of deleting it.
- **`log` warns about orphaned legacy stores.** When a legacy store is detected, `log` prints one ASCII-only note to stderr: `note: legacy project-local feedback store at <path> is no longer read; merge it into <active root> (one-time)`. Exit code and stdout JSON are unchanged -- this is stderr-only, informational.

### Fixed
- **plugin.json manifest drift corrected.** The manifest still said `0.4.0` while this CHANGELOG's latest release is 0.5.0, so every envelope (`tool.version`) has been self-reporting 0.4.0 since 2026-05-02. Manifest now says 0.5.0. Its `description` also still claimed project-local `.agent-plus/skill-feedback/` storage -- updated to the user-global path. README's worked example is realigned to the real envelopes (`logged`/`path` + `nextSteps` on `log`; `since`/`rating_histogram`/`top_friction[].text`/`provenance` on `report`), and the `submit` repo-resolution claim drops the `homepage` fallback that v0.3.0 removed.
- **SKILL.md path claims realigned.** The frontmatter `description` and the intro still said entries land in project-local `.agent-plus/skill-feedback/` "next to the project"; both now say the user-global `~/.agent-plus/skill-feedback/` path (same drift class as the plugin.json description). The storage example's illustrative `tool_version` is corrected from the never-released `0.4.1` to `0.5.0`, and the "What it doesn't do" purge note uses the global path.
- **`_git_toplevel()` now normalises MSYS-style paths on Windows** (`/c/dev/foo` -> `C:/dev/foo`) via the same `_msys_to_windows()` helper already used in `.claude/hooks/suggest-repo-analyze.py` and `agent-plus-meta/bin/_subcommands/init.py`. `_git_toplevel()` had zero call sites before this release; wiring it into legacy-store detection without the guard would have reintroduced the exact wrong-path bug TODOS.md's "Windows path audit" item already documents for this same `git rev-parse --show-toplevel` call.
- **The draft Stop hook (`.claude/hooks/check-skill-feedback.sh`) still implemented the old 4-tier resolution.** Its `storage_root()` did env -> git-toplevel -> cwd -> home under a comment claiming it resolves "the same way the CLI does" -- false since v0.19.4. Consequence for an opted-in user: the freshness check watched project-local `*.jsonl` files the CLI never writes, so the end-of-session nudge fired forever even after feedback was logged. Now env -> home only, mirroring the CLI; header comment and the marker-file docs (`<project-or-home>/...` -> `$HOME/...`, noting the `$SKILL_FEEDBACK_DIR/.enabled` override) updated to match, along with README's "Optional Stop hook" marker path.
- **Marketplace listing + docstring intro still claimed project-local storage.** `.claude-plugin/marketplace.json`'s skill-feedback `description` (the text every `claude plugin marketplace add` user sees) and the `bin/skill-feedback` module-docstring intro paragraph both still said entries land in `.agent-plus/skill-feedback/<skill>.jsonl`; both now say `~/.agent-plus/skill-feedback/<skill>.jsonl` (same drift class as the plugin.json description fix above).

### Tests
- `test_home_fallback_when_no_git_no_marker` renamed to `test_home_wins_even_inside_git_repo` and tightened: now `git init`s a real temp repo and asserts `source == "home"` outright (previously hedged with `assertIn(data["source"], ("git", "cwd", "home"))`, since those sources could no longer occur) -- the regression test proving repo-local storage precedence is gone.
- New `TestLegacyStoreDetection` (3 cases): `path` reports `legacy_store` when a pre-v0.19.4 dir with a `.jsonl` file is present; `log` emits the stderr note while still writing to the home store; negative case (no legacy dir) reports `null` and prints no note.
- Suite: 50 -> 53.
