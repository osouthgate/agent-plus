...
def test_scaffold_generated_skill_scrub_text_parity(tmp_path: Path):
    ...
    assert "[REDACTED]" in out
    assert "Authorization" in out
    # Scaffolded skills must also carry the 2026-07 additions (Netlify PAT +
    # env-var-assignment). Locks the template<->canonical sync that this
    # commit restored: the template had drifted 7 patterns behind.
    fake_pat = "nfp_" + "z" * 36
...
# ─── 2026-07 incident: env-assignment secrets + Netlify PAT ───────────────────
#
# skill-plus scan mined `NETLIFY_AUTH_TOKEN=nfp_<...> bunx netlify-cli` from
# session history and wrote it UNREDACTED to candidates.jsonl: no pattern
# matched the `VAR=secret` shape, and `nfp_` wasn't a known token prefix.
# All tokens below are FAKE.


def test_field_repro_netlify_deploy_command_redacted():
    """The exact leaked shape: a NETLIFY_AUTH_TOKEN=... env-assignment prefix
    on a bunx netlify-cli deploy one-liner. Value gone, NAME + command kept."""
    mod = _load_bin_module()
