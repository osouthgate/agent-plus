...
### Added
- **`agent-plus-upgrade` skill.** `upgrade-check` and `upgrade` have shipped fully built since v0.13.5 (snooze ladder, `.bak` rollback, migrations, post-upgrade doctor gate) but nothing ever called `upgrade-check` proactively — it wasn't wired into any `nextSteps` chain, so every installed user silently drifted from the latest release with no signal. New pure-markdown skill at `skills/agent-plus-upgrade/SKILL.md` (mirrors `agent-plus-installer`'s shape) with two trigger paths: **explicit** (user directly asks "is agent-plus up to date" — always answers, no cap) and **ambient** (once per session, after the first agent-plus skill/tool call finishes its own work, gated on an `AGENT_PLUS_UPGRADE_CHECKED` session flag). Either path probes via `upgrade-check`; an unsolicited offer only surfaces on `verdict: "upgrade_available"` with `config.update_check` still `true` and no active snooze. No CLI changes; wraps the existing frozen contract verbatim — live-verified against the real installed 0.19.6 binary (genuinely stale against the 0.21.0 release), confirming the envelope's field paths (`verdict`, `config.update_check`, `snooze.active`) match exactly. Covered by `test/test_upgrade_skill.py`.
- **`envcheck`'s clean-path `nextSteps` now also chains to `upgrade-check`.** Complements the skill above with reach into CLI-direct and bootstrap-running sessions that never invoke a skill at all. Appended as entry #2 (`_history_branched_step()` stays at index 0 — the onboarding first-win step keeps priority); the dirty path (`missing` non-empty) is unchanged. `doctor`'s own clean path was considered and rejected: `doctor` → `upgrade-check` → (not-available) `doctor --pretty` would be a genuine cycle. `refresh` was also considered but `envcheck` wins on coverage — it's local/free/doctrine-mandated every session, where `refresh` is network- and token-gated and may not run at all in an offline or no-token session. `_inject_next_steps()` in `bin/agent-plus-meta`; covered by two new cases in `test/contract/test_next_steps_contract.py`.

### Changed
- **Skill-picker findability.** Both SKILL.md files in this plugin (`agent-plus-meta`, `agent-plus-installer`) now prefix their frontmatter `description` with `agent-plus | ` so typing "agent-plus" in Claude Code's skill picker surfaces the whole suite instead of six generically-named skills.
...
### Fixed
- **`--json` placed AFTER the subcommand (the documented script-mode spelling) was silently ignored on a TTY** for `upgrade-check`, `upgrade`, and `uninstall`: each subparser declared its own `--json` with `dest="json"`, which `main()` never read (it reads only `force_json`), so `agent-plus-meta uninstall --json` from a terminal-attached script emitted no envelope at all. All three subparser flags now share `dest="force_json"` -- and carry `default=argparse.SUPPRESS`, which is load-bearing: argparse (3.7+) parses subparser args into a fresh namespace and copies every attribute back over the parent's, so a plain `store_true` default would clobber a `--json` given BEFORE the subcommand (empirically confirmed on CPython 3.12). Both positions are now contract-tested against a fake TTY (`test_uninstall.py::TestJsonFlagPositions`, `test_next_steps_contract.py::TestJsonFlagBothPositionsTTY`).
- **`uninstall --json` (and any non-TTY-stdin run) could block on the y/N scope confirmation**: the prompt was gated only on `--non-interactive`, so automation holding a pipe open hung forever and closed-stdin runs burned a prompt nobody could answer. The scope confirmation now fires only when genuinely interactive -- `(not --non-interactive) and (not --json) and stdin.isatty()` -- and skipped-prompt runs proceed with exactly the auto-confirmed semantics `--non-interactive` has always had. Note the flip side: piping `y`/`n` into the prompt no longer does anything (non-TTY runs never read the pipe); select scope with flags. The envelope's `interactive` field now records this actual gating rather than restating the flag set.
- **`_git_toplevel()` mis-rooted MSYS/Git-Bash output.** `git rev-parse --show-toplevel` under Git Bash emits `/c/dev/foo`; `Path()` on Windows turned that into `C:\c\dev\foo` (same incident class as the repo-analyze stamp bug in TODOS.md's "Windows path audit"). Git output is now routed through the accepted `_msys_to_windows()` idiom (copied from `_subcommands/init.py` / skill-feedback) before `Path()`; helper- and wrapper-level unit tests added (`test_msys_paths.py`).

### Changed
- **`--purge` now requires a real interactive terminal.** When `--purge` is combined with `--json`, `--non-interactive`/`--auto`, or a non-TTY stdin, the run is refused up front with the standard structured error envelope on stderr and exit code 1 (`"purge requires an interactive terminal; run without --purge or confirm at a TTY"`) -- instead of firing the PURGE prompt anyway (hang on held-open stdin, dead prompt at EOF). This enforces the already-documented principle that `--auto`/`--non-interactive` never bypass the purge confirmation; it is not a new door. `--dry-run --purge` remains allowed in every mode (never prompts, never removes). Docs: `docs/uninstall-envelope.md` "Script mode" section. Tests updated to the new contract: `test_uninstall_purge_under_non_interactive_still_prompts` is now `test_uninstall_purge_under_non_interactive_refuses`, and the literal-PURGE tests fake a TTY stdin (the prompt now only reaches genuinely interactive sessions).
